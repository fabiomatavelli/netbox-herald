...
var _ = Describe("Service Controller", func() {
	...
	It("no-ops until HeraldConfigReconciler has populated the Store", func() {
		...
		// No Service named "does-not-exist" exists either, but the Store
		// check happens first, so this must return cleanly without ever
...
var _ = Describe("cidrForIP", func() {
	It("renders an IPv4 address with a /32 host mask", func() {
		cidr, err := cidrForIP("10.13.11.50")
		Expect(err).NotTo(HaveOccurred())
		Expect(cidr).To(Equal("10.13.11.50/32"))
	})

	It("renders an IPv6 address with a /128 host mask", func() {
		cidr, err := cidrForIP("2001:db8::1")
		Expect(err).NotTo(HaveOccurred())
		Expect(cidr).To(Equal("2001:db8::1/128"))
	})

	It("rejects an invalid address", func() {
		_, err := cidrForIP("not-an-ip")
